...
function createMap(payload) {
    var map, k;
    map = new solacePool.SDTMapContainer();
        ...
        if (!connections[id]) {
            connections[id] = function() {
                    ...
                    for (var s in subscriptions) {
                        if( checkTopic(subscriptions[s].topic, destination)) {
                            ...
                            if(message.getType() === solacePool.MessageType.MAP) {
                                tempMsg = {};
                                map = message.getSdtContainer().getValue();
                            ...
                            if(message.getType() === solacePool.MessageType.STREAM ) {
                                var count = 0;
                                tempMsg = [];
                                stream = message.getSdtContainer().getValue();
                ...
                var eventCallback = new solacePool.SessionEventCBInfo(function(session, event) {
                    var topic, payload, msgtype, pubTopic, message, map, stream;
                    var p, s, r;
                    ...
                    if (event.sessionEventCode === solacePool.SessionEventCode.UP_NOTICE) {
                        ...
                        for (r in registerStatus) {
                            registerStatus[r].statusCallback("connected");

                        }
                        
                        for (s in subscriptions) {
                            topic = subscriptions[s].topic;
                        ...
                        for (p in publications) {
                            for( s in subscriptions) {
                                if(!(checkTopic(subscriptions[s].topic, publications[p].topic))) {
                                    topic = publications[p].topic;
                                    payload = publications[p].payload;
                                    msgtype = publications[p].msgtype;
                                    pubTopic = solacePool.SolclientFactory.createTopic(topic);
                                    message = solacePool.SolclientFactory.createMessage();
